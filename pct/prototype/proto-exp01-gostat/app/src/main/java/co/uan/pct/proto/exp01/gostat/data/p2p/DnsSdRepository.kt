...
import co.uan.pct.proto.exp01.gostat.util.PctCtrlRecordFactory
import co.uan.pct.proto.exp01.gostat.util.PctInstanceCodec
import co.uan.pct.proto.exp01.gostat.util.PctTxtParser
...
class DnsSdRepository(
    ...
    private fun onDiscoverySettle() {
        ...
        emitEvent(
            "Ventana de escaneo cerrada (${T_CANDIDATE_SETTLE_MS}ms): " +
                "$count padre(s) candidato(s) — elige uno; sin auto-conectar",
        )
        updateDiagnostics {
            it.copy(
                hint = if (count == 0) {
                    "Ningún padre usable; reintenta o inicia como raíz"
                } else {
                    "Elige un padre de la lista (1 solicitud STA máx.)"
                },
            )
        }
        lockDiscovery("Escaneo finalizado")
    }

    private fun lockDiscovery(reason: String) {
    ...
    private fun scheduleCandidateSettle() {
        if (discoveryLocked) return
        mainHandler.removeCallbacks(settleRunnable)
        mainHandler.postDelayed(settleRunnable, T_CANDIDATE_SETTLE_MS)
    }
